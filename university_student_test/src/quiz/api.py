...
async def update(
    ...
    return await service.update_quiz(quiz_id=quiz_id, quiz_data=quiz_data)
...
async def delete(
    quiz_id: int,
    service: QuizService = Depends(get_quiz_service),
    _ : TokenPaylod = Depends(require_permission("delete:quiz"))
):
    await service.delete_quiz(quiz_id=quiz_id)
    return {
